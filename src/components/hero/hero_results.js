...
class HeroDetail extends Component {
    ...
    onClickHandler(hero) {
        if(hero) {
            ...
            alert("We added a super hero, go and see the 'my heroes' tab. \n\n - sorry for this horrible alert, need more UX/UI :P -");
        }
    }
            ...
            return heroes.map((hero)=> {
                return (
                    <div className="card" key={hero.id}>
                        <HeroItem name={hero.name} image={`${hero.thumbnail.path}.${hero.thumbnail.extension}`} />
                        <button onClick={this.onClickHandler.bind(this, hero)}><i className="fa fa-plus" aria-hidden="true"></i></button>
                    </div>
                );
            });
        }
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({add_player : player_add_action}, dispatch);
}
